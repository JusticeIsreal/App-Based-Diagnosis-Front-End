...
const Step1 = () => {
    ...
    const getCategories = () => {
        const search = localStorage.getItem("clinical")

        if (!search)
            router.push("/my/clinical")
        ...
        axios.post("https://abcdmedical.herokuapp.com/api/v1/diagnosis/clinical/singlecat", body ,config )
        .then(res => {
    ...
    const forGuide = (id) => {
        ...
        try {
            localStorage.setItem("clinical-guide", id)
            router.push("/my/clinical/step-2")
    ...
    const forCategory = (name) => {
        ...
        try {
            localStorage.setItem("clinical", name)
